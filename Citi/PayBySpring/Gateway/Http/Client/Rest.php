 ...
 */
namespace Citi\PayBySpring\Gateway\Http\Client;
...
class Rest implements ClientInterface
{
    ...
    public function placeRequest(TransferInterface $transferObject)
    {
        ...
        try {
            ...
            $credential = $transferObject->getAuthUsername() . ":" . $transferObject->getAuthPassword();
            $headers[] = sprintf('%s: %s', "Authorization","Basic " . base64_encode($credential));

            $log['headers'] = $headers;
            $this->adapter->write(
                $transferObject->getMethod(),
                \Zend_Uri_Http::fromString("http://citi.mtf.gateway.mastercard.com/api/rest/version/58/merchant/202005111567/session"),
                self::HTTP_1,
                $headers,
